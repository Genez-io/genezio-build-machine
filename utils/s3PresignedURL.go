...
func UploadContentToS3(archivePath, projectName, region, stage, token string) (string, error) {
	...
	file_buf := make([]byte, f_stat.Size())
	_, err = f.Read(file_buf)
	if err != nil {
		return "", err
	}

	reqUpload, err := http.NewRequest("PUT", presignedURL, bytes.NewBuffer(file_buf))
	if err != nil {
		return "", err
	}

	fileSizeStr := fmt.Sprintf("%d", f_stat.Size())
