...
func writeCodeMapToDirAndZip(code map[string]string, tmpFolderPath string) (string, error) {
	...
	err := utils.ZipDirectory(tmpFolderPath, destinationPath)
	if err != nil {
		return "", err
	}
...
func (d *deploymentsController) DeployFromS3Workflow(w http.ResponseWriter, r *http.Request) {
	...
	projectName := "express-getting-started"
	if body.ProjectName != nil {
	...
	archive_path, err := writeCodeMapToDirAndZip(body.Code, tmpFolderPath)
	if err != nil {
	...
	s3URL_upload, err := utils.UploadContentToS3(archive_path, projectName, region, "prod", body.Token)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	// Parse upload s3 url to extract key
	s3ParsedURL, err := url.Parse(s3URL_upload)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	s3URL_download, err := utils.DownloadFromS3PresignedURL(region, fmt.Sprintf("%s-%s", bucketBaseName, region), uploadKey)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}

	workflow_res := d.wfService.S3Workflow(body.Token, s3URL_download, body.ProjectName, body.Region, body.BasePath)
	w.Write([]byte(workflow_res))
}
...
func (d *deploymentsController) DeployFromGithubWorkflow(w http.ResponseWriter, r *http.Request) {
	...
	// Call service
	workflow_res := d.wfService.GithubWorkflow(
		body.Token,
		body.Repository,
		body.ProjectName,
		body.Region,
...
func (d *deploymentsController) DeployEmptyProjectWorkflow(w http.ResponseWriter, r *http.Request) {
	...
	// Call service
	workflow_res := d.wfService.EmptyProjectWorkflow(
		body.Token,
		body.Repository,
		body.ProjectName,
		body.Region,
